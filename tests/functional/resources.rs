...
use k8s_openapi::api::{
    core::v1::{Pod, Service, Namespace, Container, ContainerPort, PodSpec, PodTemplateSpec, ServiceSpec, ServicePort},
...
impl TestResources {
    ...
    pub async fn create_test_deployment(&self, name: &str, replicas: i32) -> Result<Deployment, Box<dyn std::error::Error>> {
    ...
    pub async fn create_test_service(&self, name: &str, app_selector: &str) -> Result<Service, Box<dyn std::error::Error>> {
    ...
    pub async fn wait_for_pods_ready(&self, label_selector: &str, expected_count: usize) -> Result<(), Box<dyn std::error::Error>> {
        let pods: Api<Pod> = Api::namespaced(self.client.clone(), &self.namespace);
        
        for _ in 0..120 {
            let pod_list = pods.list(&ListParams {
        ...
        Err(format!("Expected {} pods to become ready, but only found matching pods", expected_count).into())
    }

    pub async fn get_pods(&self, label_selector: &str) -> Result<Vec<Pod>, Box<dyn std::error::Error>> {
        let pods: Api<Pod> = Api::namespaced(self.client.clone(), &self.namespace);
        let pod_list = pods.list(&ListParams {
            label_selector: Some(label_selector.to_string()),
            ..Default::default()
